  ...
  const formatPreview = (content) => {
    ...
    if (Array.isArray(content)) {
      return content.map((item, idx) => (
        <span key={idx} className="inline-block bg-zinc-100 dark:bg-zinc-700 px-2 py-1 rounded text-xs mr-1 mb-1">
          {String(item)}
        </span>
      ));
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3">
              <label className="flex items-center">
                <input
                  ...
                  className="mr-2"
                />
                <span className="text-sm text-zinc-700 dark:text-zinc-300">Rows</span>
              </label>
              <label className="flex items-center">
                <input
                  type="radio"
          ...
          <div className="grid grid-cols-2 gap-3">
           ...
           <p className="text-xs text-zinc-500 dark:text-zinc-400">
             Enter indices between 0 and {swapType === 'row' ? maxRows - 1 : maxCols - 1}
           </p>
